...
def split_ssl_data(file_name):
    base_dataset = blood_mnist_SSL(split='test', download=True)
    #8872574
    index = random_split([1000, 500, len(base_dataset)-1500], 8872574) 
    ...
    while i < len(noisy_label):
        if noisy_label[i] == -1:
            ...
            i -= 1
        i+=1
    label = blood_mnist_SSL(split='test',index = index[0], noisy_targets=noisy_label)
    u_label = blood_mnist_SSL(split='test',index = index[2])
    test = blood_mnist_SSL(split='test',index = index[1])
    return label.data, label.targets, u_label.data, u_label.targets, test.data, test.targets, 

class blood_mnist_SSL(medmnist.DermaMNIST):
