...
#     # train
    ...
    all_private_dataloader = torch.utils.data.DataLoader(quire_dataset, batch_size)
    teacher_preds_all = []
    train_accs = []
    test_accs = []
    for teacher in range(num_teachers):
        print("############################### Teacher {} Model Training #############################".format(teacher+1))
        train_acc, test_tacc, tr_model = train_model(teacher_datasets[teacher], quire_dataset, args)
    ...
    torch.save(teacher_preds_all, f"../teacher_preds/teacher_preds_ffhq.pth")  
