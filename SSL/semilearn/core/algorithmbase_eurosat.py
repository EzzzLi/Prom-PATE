...
def split_ssl_data(file_name):
    base_dataset = myEuroSAT("../data/EuroSAT",  download=True)
    #8872574
    index = random_split([10000, 1000, 1000, len(base_dataset)-12000], 8872574) 
    noisy_label = torch.load(f"../noisy_label/{file_name}.pth").numpy().tolist()
    i = 0
    index[3].extend(index[2])
    while i < len(noisy_label):
        if noisy_label[i] == -1:
            ...
            i -= 1
        i+=1
    label = myEuroSAT("../data/EuroSAT", index = index[2], noisy_targets=noisy_label, download=True)
    u_label = myEuroSAT("../data/EuroSAT", index = index[3],download=True)
    test = myEuroSAT("../data/EuroSAT", index = index[1],download=True)
