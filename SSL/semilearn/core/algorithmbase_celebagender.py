...
def split_ssl_data(file_name):
    base_dataset = Celeba_gender('../data/CelebA', split='test', download=True)
    #8872574
    index = random_split([3000, 1000, len(base_dataset)-4000], 8872574) 
    ...
    while i < len(noisy_label):
        if noisy_label[i] == -1:
            ...
            i -= 1
        i+=1
    label = Celeba_gender('../data/CelebA',split='test',index = index[0], noisy_targets=noisy_label)
    u_label = Celeba_gender('../data/CelebA',split='test',index = index[2])
    test = Celeba_gender('../data/CelebA',split='test',index = index[1])
