...
def split_ssl_data(file_name):
    base_dataset = mySVHN('../data/SVHN', split='test', download=True)
    #8872574
    index = random_split([2000, 1000, len(base_dataset)-3000], 8872574) 
    noisy_label = torch.load(f"../noisy_label/{file_name}.pth").numpy().tolist()
    i = 0
    ...
    while i < len(noisy_label):
        if noisy_label[i] == -1:
            ...
            i -= 1
        i+=1
    label = mySVHN('../data/SVHN', index = index[0], noisy_targets=noisy_label, split='test', download=True)
    # label = mySVHN('./data/SVHN', index = index[0], split='test', download=True)
    u_label = mySVHN('../data/SVHN', index = index[2], split='test', download=True)
    test = mySVHN('../data/SVHN', index = index[1], split='test', download=True)
    return label.mydata, label.targets, u_label.mydata, u_label.targets, test.mydata, test.targets, 
    ...
    def __init__(self, root, index = None, noisy_targets = None,split = "train", transform= None, target_transform = None, download = False) -> None:
        ...
        if index is not None:
            for i in self.index:
                ...
                self.mydata.append(img)
            self.mydata = np.array(self.mydata)
