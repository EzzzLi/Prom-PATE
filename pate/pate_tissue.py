...
class my_medmnist(medmnist.TissueMNIST):
...
class reProgrammingNetwork(nn.Module):
    def __init__(self, input_size=224, patch_H_size=192, patch_W_size=192, channel_out=3, device="cpu") -> None:
        ...
        self.new_layers = nn.Sequential(nn.ReLU(),nn.Linear(1000, 8))
    ...
    def forward(self, image):
        ...
        X[:,:,self.H_start:self.H_end,self.W_start:self.W_end] = image.repeat(1,3,1,1).data.clone()
...
def main():
    ...
    torch.save(teacher_preds_all, f"teacher_preds_tissue_1000_wideresnet_192_tansfer.pth") 
    torch.save(test_accs, f"testacc_preds_tissue_1000_wideresnet_192_tansfer.pth") 
